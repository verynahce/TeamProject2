...
public class PdsFile {
	...
	public static void save( MultipartFile[] uploadfiles, HashMap<String, Object> map) {
		...
		List <PortfolioVo> fileList = new ArrayList<>();
		...
		PortfolioVo vo = new PortfolioVo(fileName,fileExt,saveName2, Integer.valueOf(map.get("resume_idx").toString()));
		fileList.add(vo);
	...
	//실제물리파일 삭제 : 여러파일 삭제
	public static void delete(String uploadPath,List<PortfolioVo> fileList) {
	String path = uploadPath;
	   fileList.forEach((file)-> {
	   String sfile = file.getFile_path(); // 실제 파일명 uuid	
...
public static void getsave(MultipartFile uploadimage, HashMap<String, Object> map) {
		
		//저장된 경로를 가져온다
				String uploadPath = String.valueOf(map.get("uploadPath"));									
				
				String orgName = uploadimage.getOriginalFilename();
				System.out.println(orgName);
				String fileName = 
						(orgName.lastIndexOf("\\")<0)
						?orgName
				...
				// 파일저장
				try {
					uploadimage.transferTo(savePath); //업로드 폴더에 파일을 저장
					System.out.println( savePath + "가 저장됨");
				} catch (Exception e) {
				e.printStackTrace();
				}; 
				...
				//저장된 파일들의 정보를 map에 list 방식으로 저장 -> pdsServiceImpl에 전달 
				ImagefileVo vo = new ImagefileVo(fileName,fileExt,saveName2, String.valueOf(map.get("type")));
		        map.put("idPhoto", vo );
		        System.out.println( map);	
		...
		//실제물리파일 삭제 : 여러파일 삭제
		public static void delete3(String uploadPath,List<ImagefileVo> fileList) {
		String path = uploadPath;
		   fileList.forEach((file)-> {
